...
class Sensor_Packet(Packet):
    ...
    def readPyld(self):
        ...
        tmp_d, tmp_p = unpack('<HH', self.pyld[20:24])
        self.temperature_d = ((tmp_d * 3.3 / 4095 * 1000) - 500) / 10
        b = 3455
        if(tmp_p == 0):
            # print('Here')
            tmp_p = 0.1
        ...
        self.pd_1 = unpack('<H', self.pyld[24:26])[0]
...
class Sweep():
    ...
    def __init__(self, pyld, nSteps):
        ...
        self.i_pd_1 = unpack('<H', pyld[4:6])
        self.f_pd_1 = unpack('<H', pyld[6:8])
    ...
    def fillVals(self):
        ...
        for i in range(self.nSteps):
            arr = unpack('<HHHH', self.pyld[(4*2*i) + 8: (4*2*i + 8) + 8])
...
class CstSweep(Sweep):
    ...
    def fillVals(self):
        ...
        for i in range(self.nSteps):
            arr = unpack('<HHHH', self.pyld[(4*2*i) + 6: (4*2*i + 8) + 6])
            #self.adcDACVal[i] = arr[0] / self.nAvg
            #print(arr[0])
    ...
    while(loc < len(raw)):
        ...
        if(verifyHeader(sync, pcktType, pyldLen)):
            if(pcktType == typeSens):
                ...
                myPackets.append(packet)
            elif(pcktType == typeDen):
                # print("Dense")
                packet = Dense_Sweep(count, tInitial, tFinal, pcktType, pyldLen, raw[(loc+lenHedr) : (loc+lenHedr+pyldLen)])
                myPackets.append(packet)
            else:
                loc+=1
                print('Missing link')
            ...
            # if(sync == 0x5544):
            #     print("Skip: ", pcktType)
            # if(sync[0] == 0x55 and sync[1] == 0x44):
                # print("Skip: ", pcktType)
                # print("*************************")
            loc+=1
...
if __name__ == '__main__':
    #Unit test cases
    packetList = readFile("UDIP0031.DAT")
    print(f"Unpacked {len(packetList)} packets!")
    #print(packet[0].accX)
    #print(packet[0].accY)
    #print(packet[0].accZ)
    accXList = []
    ...
    for packet in packetList:
        ...
        elif(packet.pcktType == typeDen):
            DenSwpList.append(packet)
    ...
    plt.plot(DenSwpList[5].sweep.adcDACVolt,DenSwpList[5].sweep.adc1Curr)
    plt.show()
    # print(lrgSwpList[0].sweep.adcDACVal)
    # print('\n')
    # print(lrgSwpList[0].sweep.adc1Curr)
